...
Description: Let your visitors share a post/page with others. Supports e-mail and posting to social bookmarking sites. Thanks to <a href="http://www.twistermc.com/">Thomas McMahon</a> for footwork on the URLs. Adaptado al castellano por <a href="http://www.artux.com.ar">Andres "Artux" Scheffer</a>
Version: 1.3
Author: Alex King
...
$social_sites = array(
	...
	, 'loturak' => array(
		'name' => 'Loturak'
		, 'url' => 'http://www.loturak.es/nuevo?enlace-url={url}&enlace-titulo={title}'
	)
	, 'meneame' => array(
            'name' => 'Meneame'
	...
	, 'fresqui' => array(
		'name' => 'Fresqui'
		, 'url' => 'http://tec.fresqui.com/post?url={url}&title={title}'
	)
	, 'neodiario' => array(
		'name' => 'Neodiario'
		, 'url' => 'http://www.neodiario.net/submit.php?url={url}'
	)
	, 'enchilame' => array(
		'name' => 'Enchilame'
		, 'url' => 'http://enchilame.com/submit.php?url={url}'
	)
...
function akst_share_link($action = 'print') {
	...
	ob_start();
?>
<a href="<?php bloginfo('siteurl'); ?>/?p=<?php print($post->ID); ?>&amp;akst_action=share-this" <?php print($onclick); ?> title="<?php _e('E-mail this, post to del.icio.us, etc.', 'alexking.org'); ?>" id="akst_link_<?php print($post->ID); ?>" class="akst_share_link"><?php _e('Compartelo', 'alexking.org'); ?></a>
<?php
...
?>
	<!-- Share This BEGIN -->
	<div id="akst_form">
		<a href="javascript:void($('akst_form').style.display='none');" class="akst_close"><?php _e('Cerrar', 'alexking.org'); ?></a>
		<ul class="tabs">
...
function akst_send_mail() {
	...
	$subject = __('Queria compartir este post contigo de ', 'alexking.org').get_bloginfo('name');

	$message = __('Saludos Cordiales--', 'alexking.org')."\n\n"
		.$name.__(' quiere compartir el siguiente articulo contigo:', 'alexking.org')."\n\n"
		.$url."\n\n"
		.__('Disfrutalo.', 'alexking.org')."\n\n"
		.'--'."\n"
		.get_bloginfo('home')."\n";
